...
Both systems can hold data about the same entity. For example, after a call the agent writes the deal amount and stage to SQLite (structured record), and simultaneously extracts the customer's tone, urgency, and key concerns into mem0ai (semantic recall). The agent uses mem0ai for "what do I remember about this customer" and SQLite for "what's the current pipeline state."
...
The agent uses the SQLite plugin's `create_table` and `insert_sql` to initialize these tables on first use:
...
These rules guide the agent's decision about which storage layer(s) to use:
